<?php
	 $data['min_price'] = (isset($_REQUEST['min_price']) && $_REQUEST['min_price']!='')?$_REQUEST['min_price']:$prRange['min_price'];
	 $data['max_price'] = (isset($_REQUEST['max_price']) && $_REQUEST['max_price']!='')?$_REQUEST['max_price']:$prRange['max_price'];
...
<form role="search" method="get" id="quick_search" class="woocommerce-product-search" action="<?php echo esc_url( home_url( '/'  ) ); ?>">
  ...
  <div class="product">
    <label class="screen-reader-text" for="s">Product Category</label>
    <select class="radius" name="product_cat">
      ...
      <option value="spirit" <?php if(isset($data['cat']) && $data['cat']=='spirit') echo 'selected="selected"'; ?>>Spirit</option>
  ...
  <div class="product">
    ...
    <label class="screen-reader-text" for="s">Your Location <a href="javascript: void(0);" style="font-size:12px; float:right; margin-left:80px; text-decoration:underline; " onclick="findCurLoc();"><i>Locate me</i></a></label>
    <input type="search" class="search-field" placeholder="Zip…/ Address" value="<?php echo $zipcode; ?>" id="zipcode" name="zipcode">
    <label class="screen-reader-text" for="s"><br>Nearest by</label>
  ...
  <div class="product noPadding"> <a href="<?php echo site_url(); ?>/advanced-search/" class="advanced">Advanced Search</a> </div>
</form>
...
<script>
	...
	$(function() {
		$( "#slider-range" ).slider({
			...
			max: <?php echo (isset($prRange['max_price']) && $prRange['max_price']!='')?$prRange['max_price']:3000; ?>,
			values: [ <?php echo (isset($data['min_price']) && $data['min_price']!='')?$data['min_price']:0; ?>, <?php echo (isset($data['max_price']) && $data['max_price']!='')?$data['max_price']:3000; ?> ],
	...
	function successFunction(position) { 
	   ...
	   jQuery('#singlebannerwidget-2 .banner_wrapper').hide();
	   jQuery('#singlebannerwidget-2 h3').empty().html('Your Location');
	   jQuery('#singlebannerwidget-2').append('<div id="user-location-current" style="height:195px;"></div>');
...
function validateNumeric() {
	...
	if (!Number(val)) {
		...
		document.getElementById("max_price").value = '';
		alert('Invalid value, Please enter min price value from 0 - 3000');
	}
}		
function validateMinNumeric() {
	...
	} else if ((val < 0 || val > 2999)) {
		document.getElementById("min_price").value = '';
		alert('Invalid value, Please enter max price value from upto 3000');
	}
}		
</script>
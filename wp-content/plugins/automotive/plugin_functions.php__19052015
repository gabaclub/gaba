...
if(!function_exists("inventory_listing")){
	function inventory_listing($id, $layout = "fullwidth", $only_sold = false){ 
		global $lwp_options, $slider_thumbnails;
		...
		//if ( $only_sold && (!isset($post_meta['car_sold']) || empty($post_meta['car_sold'])) ) {
			//return;
		//}
		
		if(is_page('sold-cars')){
			
					if ( $only_sold && (!isset($post_meta['car_sold']) || empty($post_meta['car_sold'])) ) {
						return;
					}
		}else{
			       if ((isset($post_meta['car_sold']) || $post_meta['car_sold'])>0 ) {
						return;
					}
		}
		...
		if(isset($gallery_images) && !empty($gallery_images) && isset($gallery_images[0])){
			...
			//var_dump($slider_thumbnails['listing']['width'], $slider_thumbnails['listing']['height']);exit;
			$full_image = wp_get_attachment_image_src( $gallery_images[0], 'auto_slider' );
			$image_src = aq_resize ($full_image[0], $slider_thumbnails['listing']['width'], $slider_thumbnails['listing']['height'], true);
	    ...
	    <div class="inventory clearfix margin-bottom-20 styled_input <?php echo (isset($post_meta['car_sold']) && $post_meta['car_sold'] == 1 ? "car_sold" : ""); ?>"> 
			...
			<?php /*?><a class="inventory anc-gallery<?php echo (isset($listing_options['badge_text']) && !empty($listing_options['badge_text']) ? " has_badge" : ""); ?>" href="javascript:void(0);" onClick="jQuery('#<?php echo $id; ?> img:first').click();"><?php */?>
            <a class="inventory<?php echo (isset($listing_options['badge_text']) && !empty($listing_options['badge_text']) ? " has_badge" : ""); ?>" href="<?php echo get_permalink($id); ?>">
	            ...
	            <img src="<?php echo $image_src; ?>" class="preview" alt="<?php _e("preview", "listings"); ?>" <?php echo (isset($lwp_options['thumbnail_slideshow']) && $lwp_options['thumbnail_slideshow'] && 0 == 1 ? 'data-id="' . $id . '"' : ""); ?>>
	            ...
	            if(count($listing_details) > 6){
	            	$first_details  = array_slice($listing_details, 0, 6, true);
	            	$second_details = array_slice($listing_details, 6, count($listing_details), true);
	            } else {
	            	$single_table  = true;
	            	$first_details = $listing_details;
	            }

	            if(isset($post_meta['car_sold']) && !empty($post_meta['car_sold'])){
	            	echo "<span class='sold_text'>" . __("Sold", "listings") . "</span>";
	            } 

	            echo "<table class='options-primary'>";
	            foreach($first_details as $detail){
	            	$safe_handle = str_replace(" ", "_", mb_strtolower($detail['singular']));
	            	...
	            	echo "<tr>";
	            		echo "<td class='option primary'>" . wpml_category_translate($detail['singular'], "singular", $detail['singular']) . ": </td>";
	            		echo "<td class='spec'>" . wpml_term_translate($detail['singular'], $value, array_search($value, $detail['terms'])) . "</td>";
	            	echo "</tr>";
	            }
	            echo "</table>";
	            ...
	            if(!isset($single_table)){
		            ...
		            foreach($second_details as $detail){
		            	...
		            	echo "<tr>";
		            		echo "<td class='option secondary'>" . wpml_category_translate($detail['singular'], "singular", $detail['singular']) . ": </td>";
		            		echo "<td class='spec'>" . wpml_term_translate($detail['singular'], $value, array_search($value, $detail['terms'])) . "</td>";
		            	echo "</tr>";
		            }
		            echo "</table>";
	            ...
	            <a href="<?php echo get_permalink($id); ?>"><div class="view-details gradient_button"><i class='fa fa-plus-circle'></i> <?php _e("View Details", "listings"); ?></div></a>
				...
				<?php if(isset($listing_options['price']['value']) && !empty($listing_options['price']['value'])){
		            ...
		            <div class="price <?php echo (isset($lwp_options['price_text_replacement']) && !empty($lwp_options['price_text_replacement']) ? 'custom_message' : ''); ?>">
		            	<?php if(isset($lwp_options['price_text_replacement']) && !empty($lwp_options['price_text_replacement'])){ ?>
		            		<?php echo $lwp_options['price_text_replacement']; ?>
		            	<?php } else { ?>
			                ...
			                <?php echo (isset($lwp_options['tax_label_box']) && !empty($lwp_options['tax_label_box']) ? '<div class="tax">' . $lwp_options['tax_label_box'] . '</div>' : ''); ?>
		                <?php } ?>
		            </div>      
	            <?php } ?>
            ...
            <?php /*?><div class="temp-galJPM" style="display:none;" id="<?php echo $id; ?>">
		<?php
			...
			$gallery_images = get_post_meta($id, "gallery_images", true);
				...
				if(!empty($gallery_images)){
					...
					foreach($gallery_images as $gallery_image){
						...
						$full 			= $full[0];
						//$full_images  .= "<a class=\"fancybox\"  href=" . $full . " rel=".$id." ><img src=\"" . $full . "\" alt=\"\" /></a>";
						$full_images  .= "<img src=\"" . $full . "\" alt=\"\" data-full-image=\"".$full."\" />";
...
function listing_template($layout, $is_ajax = false, $ajax_array = false, $only_sold = false){ 
	if($is_ajax == false) { ?>
		<div class="inner-page row">
			<?php
	}
	        global $lwp_options, $wpdb;
							
			$args     = ($is_ajax == false ? listing_args($_GET) : listing_args($_GET, false, $ajax_array));	
	
			// Override the default posts per page value for sold cars
			if (is_page('sold-cars')) {
				//$args[0]["posts_per_page"] = -1;
				$args[0]["meta_query"][] = array("key"   => "car_sold","value" => "1");
			}else{
				$args[0]["meta_query"][] = array("key"   => "car_sold", "value" => "", "compare" => "NOT EXISTS");
			}	
			...
			foreach($listings as $listing){
	            echo inventory_listing($listing->ID, $layout, $only_sold);
	        }
...
function preview_slideshow_ajax1(){
    ...
    <div class="listing-slider" style="display:none;">	
	<!--	<a title="Close" class="fancybox-item fancybox-close" href="javascript:;" id="close_preview_area"></a> -->
		<?php echo $full_images; ?>
       <?php /*?> <section class="slider home-banner">
...
if(!function_exists("listing_view")){
	function listing_view($layout, $fake_get = null){
		...
		if (is_page('sold-cars')) {
				//$args[0]["posts_per_page"] = -1;
				$listings[0]["meta_query"][] = array("key"   => "car_sold","value" => "1");
		}else{
				$listings[0]["meta_query"][] = array("key"   => "car_sold", "value" => "", "compare" => "NOT EXISTS");
		}	
	    ...
	    echo '<div class="listing-view margin-bottom-20">';
	        echo '<div class="row">';
	            echo '<div class="col-lg-8 col-md-6 col-sm-6 col-xs-12 padding-none"> <span class="ribbon"><strong><span class="number_of_listings">' . $listings . '</span> <span class="listings_grammar">' . ($listings == 1 ? $vehicle_singular : $vehicle_plural) . '</span> ' . __('Matching', 'listings') . ':</strong></span> <ul class="ribbon-item filter margin-bottom-none">';
	            ...
	            foreach($filterable as $filter){
	            	// var_dump($filter['singular']);

	                // $get_var = str_replace(array(" ", "."), array("-", "_"), mb_strtolower($filter['singular']));
	                $get_var = str_replace(" ", "_", mb_strtolower($filter['singular']));

	                if(mb_strtolower($filter['singular']) == "year"){
	                    $get_var = "yr";
	                }
	                ...
	                if(isset($get_holder[$get_var]) && !empty($get_holder[$get_var]) && is_array($get_holder[$get_var]) && isset($get_holder[$get_var][0]) && !empty($get_holder[$get_var][0]) && isset($get_holder[$get_var][1]) && !empty($get_holder[$get_var][1])){
	                    ...
	                    $get_var  = (mb_strtolower($filter['singular']) == "year" ? "yr" : $get_var);
	                    if(isset($get_holder[$get_var]) && !empty($get_holder[$get_var])){
		                    $correct_term = get_category_correct_case($filter['singular'], str_replace(" ", "-", strtolower($get_holder[$get_var])));
		                    $correct_term = wpml_term_translate($filter['singular'], $correct_term, array_search($correct_term, $filter['terms']));
		                }

	                    $filters .= (isset($get_holder[$get_var]) && !empty($get_holder[$get_var]) ? "<li data-type='" . $get_var . "'><a href=''><i class='fa fa-times-circle'></i> " . wpml_category_translate($filter['singular'], "singular", $filter['singular']) . ": " . ($filter['compare_value'] != "=" ? $filter['compare_value'] . " " : "") . " <span>" . $correct_term . "</span></a></li>" : "");
	                }
	            }
...
function listing_dropdown_terms($name, $comparison = true, $encode_terms = false){
	...
	if(!empty($key['terms'])){
        ...
        foreach($key['terms'] as $key => $term){
        	...
        	$return .= "<option value='" . ($encode_terms === true ? str_replace(array("-", " "), array("--", "-"), mb_strtolower($term)) : $term) . "'" . selected($check, str_replace(" ", "-", mb_strtolower($term)), false ) . " data-key='" . $key . "'>" . $on_select . "</option>";
        }
    } else {
	...
	function listing_filter_sort($fake_get = null){ 
	    ...
	    <div class="clearfix"></div>
	        <form method="post" action="#" class="listing_sort">
	            <div class="select-wrapper listing_select clearfix margin-bottom-15" style="display:block;">
	                <?php               
	                ...
	                foreach($filterable_categories as $key){ 
	                    ...
	                    $rawtxt = $text    = str_replace("_", " ", $key['plural']);
						
						$wpml_term = wpml_category_translate($key['singular'], "singular", $key['singular']);
	                    ?>
	                        <div class="my-dropdown <?php echo $meta; ?>-dropdown">
                            	<?php if ( $cnt === 1 ) { ?>
								...
								?>
	                            <select name="<?php echo (mb_strtolower($key['singular']) == "year" ? "yr" : str_replace(" ", "_", mb_strtolower($key['singular']))); ?>" class="listing_filter" data-sort="<?php echo str_replace(" ", "_", mb_strtolower($key['singular'])); ?>" data-label-plural="<?php echo $key['plural']; ?>" data-label-singular="<?php echo $key['singular']; ?>" tabindex="1" <?php echo ($key['compare_value'] != "=" ? " data-compare-value='" . htmlspecialchars($key['compare_value']) . "' " : ""); ?> <?php echo (isset($key['dependancy']) && !empty($key['dependancy']) ? " data-dependancy='" . $key['dependancy'] . "'" : ""); ?> <?php echo (!empty($wpml_term) && $wpml_term != $key['singular'] ? " data-translate-term='" . $wpml_term . "'" : ""); ?>>
	                                
	                                <?php if(isset($key['dependancy']) && !empty($key['dependancy']) && mb_strtolower($key['dependancy']) != "none"){ 
	                                	// get dependancy singular
	                                	$dependancy_category = get_single_listing_category($key['dependancy']);
	                                	$dependancy_singular = $dependancy_category['singular'];
	                                	$dependancy_safe	 = str_replace(" ", "_", strtolower($dependancy_singular));
	                                	$correct_case 		 = get_category_correct_case($dependancy_singular, $_GET[$dependancy_safe]);

	                                	if(isset($_GET[$dependancy_safe]) && !empty($_GET[$dependancy_safe]) && in_array($correct_case, $dependancy_category['terms'])){

	                                  		echo "<option>" . $key['plural'] . "</option>";

	                                  		$term_id = array_search($correct_case, $dependancy_category['terms']);

	                                  		// D($key['dependancies'][$term_id]);
											if(isset($key['dependancies'][$term_id]) && !empty($key['dependancies'][$term_id])){
												// foreach option
												foreach($key['dependancies'][$term_id] as $new_key){
													if(!is_null($key['terms'][$new_key])){
														echo "<option value='" . $key['terms'][$new_key] . "'>" . $key['terms'][$new_key] . "</option>";
													}
												}
											}
	                                	} else {
		                                    echo "<option>" . __("Choose a", "listings") . " " . $dependancy_singular . "</option>"; 
		                                }
	                                } else { 
	                                	...
	                                	if(function_exists("icl_translate")){
											...
											// Get post countes for the category
											$post_ttl_count = get_total_meta( $key['singular'], current($key['terms']) );
											if ( $post_ttl_count <= 0 ) {
												$post_ttl_count = get_total_meta( $key['plural'], current($key['terms']) );
											}
								    	}  ?>
	                                    <option value=""><?php _e("All", "listings"); ?> <?php echo $text; ?></option>
	                                    <?php if(!empty($key['terms'])){
	                                        ...
	                                        foreach($key['terms'] as $term_key => $term){
	                                            ...
	                                            // $get_select = (mb_strtolower($singular) == "year" ? "yr" : str_replace(" ", "-", mb_strtolower($singular)));
												...
												// Get post countes for the category
												$post_ttl_count = get_total_meta( $meta, $term );
												if ( $post_ttl_count <= 0 ) {
													$post_ttl_count = get_total_meta( $rawtxt, $term );
												}
												$translated_term = wpml_term_translate($singular, $term, $term_key);
	                                            echo "<option value='" . $option_val . "'" . selected( $get_value, str_replace(" ", "-", mb_strtolower($term)), false ) . " data-key='" . $term_key . "'" . (!empty($translated_term) && $translated_term != $option_val ? " data-translated-term='" . $translated_term . "'" : "") . ">" . $on_select.' ('.$post_ttl_count.')' . "</option>";
	                                        }
	                                    } else {
	                ...
	                <div class="row">
	                    ...
	                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 pull-right">
	                        <ul class="form-links top_buttons">
							    ...
							    $vehicle_singular = (isset($lwp_options['vehicle_singular_form']) && !empty($lwp_options['vehicle_singular_form']) ? $lwp_options['vehicle_singular_form'] : __('Vehicle', 'listings') );
							    $vehicle_plural   = (isset($lwp_options['vehicle_plural_form']) && !empty($lwp_options['vehicle_plural_form']) ? $lwp_options['vehicle_plural_form'] : __('Vehicles', 'listings') ); ?>
	                            <li><a href="#" class="gradient_button compare"><?php echo sprintf(__("Compare <span class='number_of_vehicles'>%s</span> %s", "listings"), (isset($_COOKIE['compare_vehicles']) && !empty($_COOKIE['compare_vehicles']) ? count(explode(",", urldecode($_COOKIE['compare_vehicles']))) : 0), $vehicle_plural); ?></a></li>
	                            <?php } ?>
	                        </ul>
	                    </div>
	                </div>
	            ...
	            <div class="row">
	                ...
	                <div class="col-lg-3 col-md-3 col-sm-3 text-right xs-padding-none">
	                	...
	                	if(isset($listing_options['price']['value']) && !empty($listing_options['price']['value'])){ 
	                		...
	                		if(isset($lwp_options['price_text_replacement']) && !empty($lwp_options['price_text_replacement'])){ 
		            			echo $lwp_options['price_text_replacement'];
		            		} else { 
		                        echo '<h2>' . format_currency($listing_options['price']['value']) . '</h2>';
		                        echo (isset($lwp_options['tax_label_page']) && !empty($lwp_options['tax_label_page']) ? '<em>' . $lwp_options['tax_label_page'] . '</em>' : ''); 
			                }
	            ...
	            <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 left-content padding-left-none"> 
	                ...
	                if(!empty($gallery_images)){	
						...
						foreach($gallery_images as $gallery_image){
							...
							// Custom slider thumb images
							$full_image = wp_get_attachment_image_src( $gallery_image, 'auto_slider' );
							$gallery_thumb = aq_resize ($full_image[0], $slider_thumbnails['listing']['width'], $slider_thumbnails['listing']['height'], true);
	                    ...
	                    <div id="myTabContent" class="tab-content margin-top-15 margin-bottom-20">
	                        ...
	                        <div class="tab-pane fade" id="location">
								...
								if(!empty($latitude) && !empty($longitude)){ ?>
	                            	<div class='google_map_init' class="contact" data-longitude='<?php echo $longitude; ?>' data-latitude='<?php echo $latitude; ?>' data-zoom='<?php echo $zoom; ?>' data-scroll="false" style="height: 350px;" data-parallax="false"></div>
	                            <?php } else { ?>
	                            	<?php _e("No location available", "listings"); ?>
	                            <?php } ?>
	                        </div>
	                        <div class="tab-pane fade" id="comments">
	                            <p><?php echo (isset($post_meta['other_comments']) && !empty($post_meta['other_comments']) ? wpautop(do_shortcode($post_meta['other_comments'])) : ""); ?></p>
	                        </div>
	                    </div>
	                </div>
	            </div>
	                ...
	                <div class="side-content margin-bottom-50">
	                            ...
	                            <table class="table">
	                                <tbody>
	                                    ...
	                                    foreach($listing_categories as $key => $category){
	                                        $safe_handle = $key;//str_replace(" ", "_", strtolower($category['singular']));
	                                        $value       = (isset($post_meta[$safe_handle]) && !empty($post_meta[$safe_handle]) ? $post_meta[$safe_handle] : "");
	                                        ...
	                                        echo (mb_strtolower($value) != "none" && !empty($value) ? "<tr><td>" . $category['singular'] . ": </td><td>" . $value . "</td></tr>" : "");
	                                    } ?>
	                                </tbody>
	                            </table>
	                        </div>
	                    ...
	                    <?php if(isset($lwp_options['display_vehicle_video']) && $lwp_options['display_vehicle_video'] == 1 && !empty($listing_options['video'])){ ?>
		                    <?php 
								...
								if($url['host'] == "www.youtube.com" || $url['host'] == "youtube.com"){
									...
									echo "<br><br><iframe width=\"560\" height=\"315\" src=\"http://www.youtube.com/embed/" . $video_id . "\" frameborder=\"0\" allowfullscreen></iframe>";
								} elseif($url['host'] == "www.vimeo.com" || $url['host'] == "vimeo.com"){
									$video_id = $url['path'];
									
									echo "<br><br><iframe width=\"560\" height=\"315\" src=\"http://player.vimeo.com/video" . $video_id . "\" frameborder=\"0\" allowfullscreen></iframe>";
								}
		                    ?>
	                    <?php } ?>
	                    ...
	                    <?php if(isset($lwp_options['calculator_show']) && $lwp_options['calculator_show'] == 1){ 
							if( class_exists("Loan_Calculator") ){
								the_widget("Loan_Calculator", array("text_below" => (isset($lwp_options['calculator_below_text']) && !empty($lwp_options['calculator_below_text']) ? $lwp_options['calculator_below_text'] : ""), "rate" => $lwp_options['calculator_rate'], "down_payment" => $lwp_options['calculator_down_payment'], "loan" => $lwp_options['calculator_loan'], "price" => (isset($listing_options['price']['value']) && !empty($listing_options['price']['value']) ? $listing_options['price']['value'] : "")), array('before_widget' => '<div class="widget loan_calculator margin-top-40">', 'before_title' => '<h3 class="side-widget-title margin-bottom-25">', 'after_title' => '</h3>')); 
							}
						} ?>
                        
                        <?php
							// Custom inventory listing details
							$listing_details = get_use_on_listing_categories(); 
							...
							echo "<table class='options-primary custom-listing-details'>";
							foreach($first_details as $detail){
								$safe_handle = str_replace(" ", "_", mb_strtolower($detail['singular']));
								$value       = (isset($post_meta[$safe_handle]) && !empty($post_meta[$safe_handle]) ? $post_meta[$safe_handle] : "");
...
// 	$args['meta_query'] = array('key' => 'car_sold', 'compare' => 'EXISTS');
...
function listing_args($get_or_post, $all = false, $ajax_array = false){
	...
	$args = array(
			  ...
			  'order'            	=> (isset($get_or_post['order']) && !empty($get_or_post['order']) && $get_or_post['order'] != "undefined" ? $get_or_post['order'] : $default_orderby),
	...
	foreach($filterable_categories as $filter){
		...
		// var_dump($get_singular);
		// var_dump($get_or_post[$get_singular]);
	...
	// hide sold vehicles
	if(isset($_GET['show_only_sold']) || is_page('sold-cars') ){
		$data[] = array("key"   => "car_sold",
						"value" => "1");
	} elseif( (empty($lwp_options['inventory_no_sold']) && !isset($_GET['show_sold'])) || !is_page('sold-cars') ){
		$data[] = array("key"     => "car_sold",
						"compare" => "NOT EXISTS");
	}
...
if(!function_exists("bottom_page_box")){
	function bottom_page_box($layout = false, $load = false, $fake_get = null, $additional_vars = array()){
		global $lwp_options;
...
if(!function_exists("get_total_meta")){
	function get_total_meta($meta_key, $meta_value, $is_options = false){
		...
		if(!$is_options){
			...
			$qry = "SELECT count(DISTINCT pm.post_id)
				FROM $wpdb->postmeta pm
				JOIN $wpdb->posts p ON (p.ID = pm.post_id)
			...
			if ( is_page('sold-cars') ) {
				$qry .= " AND pm.post_id IN (
                    SELECT post_id FROM wp_postmeta WHERE meta_key = 'car_sold'
                )";
			} else if ( !is_admin() ) {
				$qry .= " AND pm.post_id NOT IN (
                    SELECT post_id FROM wp_postmeta WHERE meta_key = 'car_sold'
                )";
			}
			
			$sql = $wpdb->prepare($qry, $meta_key, $meta_value);
		} else {
			$qry = "SELECT count(DISTINCT pm.post_id)
				FROM $wpdb->postmeta pm
				JOIN $wpdb->posts p ON (p.ID = pm.post_id)
			...
			$sql = $wpdb->prepare($qry, $meta_value);
		}
					...
					<div class="carasouel-slider3" <?php echo (!empty($additional_attr) ? $additional_attr : ""); ?>>
	                    ...
	                    while ( $query->have_posts() ) : $query->the_post();
	                        ...
	                        if(isset($post_meta['gallery_images']) && !empty($post_meta['gallery_images']) && !empty($post_meta['gallery_images'][0])){
								...
								// Custom slider thumb images
								$full_image = wp_get_attachment_image_src( $gallery_images[0], 'auto_slider' );
								$thumbnail = aq_resize ($full_image[0], $slider_thumbnails['listing']['width'], $slider_thumbnails['listing']['height'], true);
		                    } else {
...
function filter_listing_results($var) {	
	...
	if (is_page('sold-cars')) {
				//$args[0]["posts_per_page"] = -1;
				$args["meta_query"][] = array("key"   => "car_sold","value" => "1");
	}else{
				$args["meta_query"][] = array("key"   => "car_sold", "value" => "", "compare" => "NOT EXISTS");
	}	
	...
	$total_matches = count(get_posts($args));
	...
	$return_array = array( "content"     => $return,
						   ...
						   "bottom_page" => bottom_page_box(false, $paged, null, array("number" => $total_matches)),
						   "args"		 => $args2
				   );
...
function generate_wpml_xml(){
    ...
    echo "&lt;wpml-config><br>
            ...
            $listing_options_translate = array("vehicle_singular_form", "vehicle_plural_form", "inventory_primary_title", "inventory_secondary_title", "sale_value", "tax_label_box", "tax_label_page", "price_text_replacement", "fuel_efficiency_text", "recent_vehicles_title", "recent_vehicles_desc", "previous_vehicle_label", "request_more_label", "schedule_test_label", "make_offer_label", "tradein_label", "pdf_brochure_label", "print_vehicle_label", "email_friend_label", "next_vehicle_label", "first_tab", "second_tab", "third_tab", "fourth_tab", "fifth_tab", "listing_comment_footer", "job_description_title", "project_details_title", "related_projects_title", "default_value_price", "default_value_city", "default_value_hwy");

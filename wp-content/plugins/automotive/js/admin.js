(function($) {
 ...
 jQuery(document).ready( function($){
	...
	// Prepare the variable that holds our custom media manager.
	 ...
	 // Bind to our click event in order to open up the new media experience.
	 $(document.body).on('click.mojoOpenMediaManager', '.add_image_gallery', function(e){ 
		 ...
		 media_frame.on('select', function(){
			...
			$.each( media_attachment, function(i, val){
				 ...
				 html    += "<span class='button delete_image'>Delete image</span> ";
				 html    += "<span class='button move_image'>Move Image</span> <input type='text' name='portfolio_links[" + id + "]' value='' placeholder='Image link'></div>";
				 html    += "<input type='hidden' name='gallery_images[]' value='" + val.id + "'>";
				 html    += "</td></tr>";

...
// Plugin Updater
require 'plugin_updater/plugin-update-checker.php';
...
function listing_fix_init(){
    ...
    /* WPML Listing Options */
    if(function_exists("icl_register_string") && isset($_GET['wpml_scan_strings'])){
        // global $lwp_options;

        // $lwp_options = get_option("listing_wp");

        // foreach(get_listing_categories() as $key => $category){
        //     icl_register_string("Automotive Listing Category", $category['singular'] . " Singular", $category['singular']);
        //     icl_register_string("Automotive Listing Category", $category['singular'] . " Plural", $category['plural']);

        //     if(!empty($category['terms'])){
        //         foreach($category['terms'] as $key => $term){
        //             icl_register_string("Automotive Listing Category", $category['singular'] . " Term " . ($key + 1), $term);
        //         }
        //     }
        // }
    }

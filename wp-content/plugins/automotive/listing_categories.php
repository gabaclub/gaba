...
function add_listing_category(){
	...
	if(isset($_GET['make_default']) && !empty($_GET['make_default'])){
		...
		update_option("listing_orderby", array($make_default, (isset($category['compare_value']) && $category['compare_value'] != "=" ? "meta_value_num" : "meta_value")));
...
<table class="wp-list-table widefat listing_categories">
	<thead>
		<tr>
			...
			<th scope="col" class="manage-column" style=""><span><?php _e("Inventory orderby", "listings"); ?></span></th>	
			<th scope="col"></td>
			<th scope="col"></td>
			<th scope="col"></td>
		</tr>
			...
			foreach($listing_categories as $key => $category){ 
				...
				if(!empty($category['singular']) && !empty($category['plural'])){
					...
					<tr id="tag-<?php echo $i; ?>" class="<?php echo ($i & 1 ? " " : ""); ?>">
						...
						<td class="">
							<?php if(isset($default_orderby[0]) && $default_orderby[0] == str_replace(" ", "_", mb_strtolower($category['singular']))){ ?>
								<?php _e("Default", "listings"); ?>
								<a href="<?php echo add_query_arg("remove_default", ""); ?>"><?php _e("Remove orderby", "listings"); ?></a>
							<?php } else { ?>
								<a href="<?php echo add_query_arg("make_default", stripslashes(urlencode(mb_strtolower($category['singular']))), full_url($_SERVER)); ?>"><?php _e("Make orderby", "listings"); ?></a>
							<?php } ?>
						</td>
...
<div class="form-field">
	...
	if($dependancy != "none" && isset($_GET['edit']) && !empty($_GET['edit'])){
		...
		foreach($parent_terms as $key => $item){
			...
			$child_items = get_single_listing_category(str_replace(" ", "_", strtolower($singular_form)));

...
class WC_Dokan_Paypal_Ap_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = __( 'Dokan PayPal Adaptive Payments', 'dokan-wc-pap' );
        $this->method_description = __( 'Pay via paypal adaptive payment', 'dokan-wc-pap' );
        $this->icon               = apply_filters( 'woocommerce_paypal_icon', DOKAN_PAYPAL_ADAPTIVE_PLUGIN_PATH . '/img/paypal-adaptive.png' );

        $title                    = $this->get_option( 'title' );
        $this->title              = empty( $title ) ? __( 'PayPal Adaptive Payments', 'dokan-wc-pap' ) : $title;
    ...
    public function admin_options() {
        ?>
        <h3><?php _e( 'Dokan PayPal Adaptive Payments', 'dokan-wc-pap' ); ?></h3>
        <p><?php _e( 'PayPal Adavtive works by sending payment at a multiple user email', 'dokan-wc-pap' ); ?></p>
        ...
        <?php else : ?>
            <div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'dokan-wc-pap' ); ?></strong>: <?php _e( 'PayPal Adaptive does not support your store currency.', 'dokan-wc-pap' ); ?></p></div>
        <?php
        endif;
    }
    ...
    function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                'title'   => __( 'Enable/Disable', 'dokan-wc-pap' ),
                'type'    => 'checkbox',
                'label'   => __( 'Enable PayPal Adaptive', 'dokan-wc-pap' ),
                'default' => 'no'
            ),
            'title' => array(
                'title'       => __( 'Title', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'This controls the title which the user sees during checkout.', 'dokan-wc-pap' ),
                'default'     => __( 'PayPal Adaptive', 'dokan-wc-pap' ),
                'desc_tip'    => true,
            ),
            'description' => array(
                'title'       => __( 'Description', 'dokan-wc-pap' ),
                'type'        => 'textarea',
                'description' => __( 'This controls the description which the user sees during checkout.', 'dokan-wc-pap' ),
                'default'     => __( 'Pay via PayPal Adaptive; you can pay with your credit card if you don\'t have a PayPal account', 'dokan-wc-pap' )
            ),
            'pa_admin_email' => array(
                'title'       => __( 'PayPal Email', 'dokan-wc-pap' ),
                'type'        => 'email',
                'description' => __( 'Please enter your PayPal email address; this is needed in order to take payment.', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
                'placeholder' => 'you@youremail.com'
            ),
            'payment_process' => array(
                'title'       => __( 'Payment System', 'dokan-wc-pap' ),
                'type'        => 'select',
                'description' => __( 'Please choose your payment system', 'dokan-wc-pap' ),
                'desc_tip'    => true,
                'default'     => 'chained',
                'options'     => array(
                    'chained'  => __( 'Chained Payment', 'dokan-wc-pap' ),
                    'parallel' => __( 'Parallel Payment', 'dokan-wc-pap' )
                )
            ),
            'single_mode' => array(
                'title'       => __( 'Mode', 'dokan-wc-pap' ),
                'label'       => __( 'Single Seller Mode (Recommended)', 'dokan-wc-pap' ),
                'type'        => 'checkbox',
                'default'     => 'yes',
                'description' => __( 'When using this mode, items from <strong>only one</strong> seller can be purchased in a single order. Works only with <strong>chained payments</strong>. It\'s a good solution for marketplace as disputes/chargebacks will be handled by the seller, not by admin.', 'dokan-wc-pap' ),
            ),
            'send_shipping' => array(
                'title'       => __( 'Shipping details', 'dokan-wc-pap' ),
                'label'       => __( 'Send shipping details to PayPal instead of billing.', 'dokan-wc-pap' ),
                'type'        => 'checkbox'
            ),
            'testmode' => array(
                'title'       => __( 'PayPal sandbox', 'dokan-wc-pap' ),
                'type'        => 'checkbox',
                'label'       => __( 'Enable PayPal sandbox', 'dokan-wc-pap' ),
                'default'     => 'no',
                'description' => sprintf( __( 'PayPal sandbox can be used to test payments. Sign up for a developer account <a href="%s">here</a>.', 'dokan-wc-pap' ), 'https://developer.paypal.com/' ),
            ),
            'appuser' => array(
                'title'       => __( 'App Username', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this payment method your need an application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
                'placeholder' => 'username'
            ),
            'apppass' => array(
                'title'       => __( 'App Password', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this payment method your need an application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
            ...
            'appsig' => array(
                'title'       => __( 'App Signature', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this payment method your need an application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
            ...
            'appid' => array(
                'title'       => __( 'API App ID', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this payment method your need an application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
            ...
            'test_appuser' => array(
                'title'       => __( 'Sandbox App Username', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this system please sign up in developer account and get your  application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
                'placeholder' => 'username'
            ),
            'test_apppass' => array(
                'title'       => __( 'Sandbox App Password', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this system please sign up in developer account and get your  application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
            ...
            'test_appsig' => array(
                'title'       => __( 'Sandbox App Signature', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'For this system please sign up in developer account and get your  application credential', 'dokan-wc-pap' ),
                'default'     => '',
                'desc_tip'    => true,
            ...
            'max_error' => array(
                'title'       => __( 'Error Message', 'dokan-wc-pap' ),
                'type'        => 'text',
                'description' => __( 'This is the error message displayed to a shopper when attempting to add too many seller items to the cart due to PayPal limitation.', 'dokan-wc-pap' ),
                'default'     => __( 'Cart item quantity total exceeded - item not added to cart. Please checkout to purchase the items in your cart.', 'dokan-wc-pap' ),
                'desc_tip'    => true,
            ),
            'debug' => array(
    ...
    public function admin_script( $script ) {
        ...
        <script type="text/javascript">
            jQuery(function($) {
                $('#woocommerce_dokan_paypal_adaptive_payment_process').on('change', function() {
                    var self = $(this);
                ...
                $('#woocommerce_dokan_paypal_adaptive_payment_process').trigger('change');
            });
        </script>
        <?php
    }
    ...
    function successful_request() {
        ...
        if ( !isset( $order->id ) || empty( $order->id ) || absint( $order->id ) <= 0 ) {
            $this->add_log( sprintf( __( 'No order found by pay key: %s', 'dokan-wc-pap' ), $paykey ) );
            exit;
        }
        ...
        case 'completed' :
            ...
            $order->add_order_note( __( 'IPN payment completed', 'dokan-wc-pap' ) );
        ...
        case 'voided' :
            // Order failed
            $order->update_status( 'failed', sprintf( __( 'Payment %s via IPN.', 'dokan-wc-pap' ), strtolower( $posted['status'] ) ) );
        ...
        case 'created' :
            // order pending
            $order->update_status( 'on-hold', sprintf( __( 'Payment %s via IPN. Order is pending completion. PayPal will notify this site of further status updates. Or check your PayPal account for further information', 'dokan-wc-pap' ), strtolower( $_POST['status'] ) ) );
        ...
        case "refunded" :
            ...
            if ( $order->get_total() == ( $posted['mc_gross'] * -1 ) ) {
                ...
                $order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.', 'dokan-wc-pap' ), strtolower( $posted['status'] ) ) );

                $mailer  = WC()->mailer();
                $message = $mailer->wrap_message( __( 'Order refunded/reversed', 'dokan-wc-pap' ), sprintf( __( 'Order %s has been marked as refunded - PayPal reason code: %s', 'dokan-wc-pap' ), $order->get_order_number(), $posted['reason_code'] ) );

                $mailer->send( get_option( 'admin_email' ), sprintf( __( 'Payment for order %s refunded/reversed', 'dokan-wc-pap' ), $order->get_order_number() ), $message );
            }

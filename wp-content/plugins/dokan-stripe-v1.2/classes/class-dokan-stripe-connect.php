...
class Dokan_Stripe_Connect extends WC_Payment_Gateway {
    ...
    function payment_scripts() {
        ...
        if ( $this->stripe_checkout ) {
            ...
            wp_enqueue_script( 'woocommerce_stripe', plugins_url( 'assets/js/stripe.js', dirname( __FILE__ ) ), array( 'jquery','stripe' ), false, false );
        ...
        try {
            ...
            foreach ($all_orders as $tmp_order) {
                ...
                if ( $do_order->order_total == 0 ) {
                    $tmp_order->add_order_note( sprintf( __( 'Order %s payment completed', 'dokan-stripe' ), $tmp_order->get_order_number() ) );
                    continue;
                }
